...
namespace BoilerplatePro.Tests
{
    public abstract class BoilerplateProTestBase : AbpIntegratedTestBase<BoilerplateProTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected BoilerplateProTestBase()
        {
        ...
        protected void UsingDbContext(Action<BoilerplateProDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<BoilerplateProDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<BoilerplateProDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<BoilerplateProDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<BoilerplateProDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<BoilerplateProDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<BoilerplateProDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<BoilerplateProDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<BoilerplateProDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<BoilerplateProDbContext, Task<T>> func)
        {
            T result;
